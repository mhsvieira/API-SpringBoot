package com.vieira.crud.api.controllers;
...
import com.vieira.crud.api.responses.Response;

@RestController
@RequestMapping(path = "/api/clientes")
public class ClienteController {
	
	@Autowired // instancia classe ClienteService de forma automática
	...
	@PostMapping
	public ResponseEntity<Response<Cliente>> cadastrar(@Validated @RequestBody Cliente cliente, BindingResult result) {
		if (result.hasErrors()) {
	...
	@PutMapping(path = "/{id}")
	public ResponseEntity<Response<Cliente>> atualizar(@PathVariable(name = "id") String id, @Validated @RequestBody Cliente cliente, BindingResult result) {
		if (result.hasErrors()) {
			List<String> erros = new ArrayList<String>();
			result.getAllErrors().forEach(erro -> erros.add(erro.getDefaultMessage()));
			return ResponseEntity.badRequest().body(new Response<Cliente>(erros));
